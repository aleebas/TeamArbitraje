...
# PANEL DE CONTROL HTML Aplanado
panel_html = f"<div class='dashboard-panel'><p style='text-align:center;margin:0 0 10px 0;font-size:14px;color:#6b7280;font-weight:900;'>🎛️ PANEL DE CONTROL ({cuenta_activa})</p><div style='display:flex;justify-content:space-between;margin-bottom:10px;'><div style='width:48%;'><p style='margin:0;font-size:12px;color:var(--text-color);'>Diario BDV ($7K): <b style='color:#0ea5e9;'>${consumo_dia:,.0f} / Disp. ${c_dia:,.0f}</b></p><div class='progress-bg'><div class='progress-fill-day' style='width:{pct_dia}%;'></div></div></div><div style='width:48%;'><p style='margin:0;font-size:12px;color:var(--text-color);'>Ciclo ($10K): <b style='color:#a855f7;'>${consumo_mes:,.0f} / Disp. ${c_mes:,.0f}</b></p><div class='progress-bg'><div class='progress-fill-month' style='width:{pct_mes}%;'></div></div></div></div><p style='text-align:center;font-size:13px;margin:10px 0 0 0;color:var(--text-color);'>Vueltas: <b style='color:#10b981;'>{vueltas}</b> | Ganancia Cta: <span style='color:#10b981;'>Bs.{gan_acum:,.2f}</span> | 🌍 GLOBAL: <span style='color:#f59e0b;'>Bs.{gan_tot:,.2f}</span></p></div>"
...
if not df_h_g.empty:
    ...
    breakdown_html += "<div style='margin-top:15px; border-top:1px dashed rgba(128,128,128,0.2); padding-top:12px;'><p style='text-align:center; font-size:11px; color:#6b7280; font-weight:800; margin-bottom:8px;'>📊 DESGLOSE POR CUENTA (HOY)</p>"
    ...
    breakdown_html += "</div>"
    c_roi_g = '#10b981' if p_roi>=2 else '#ef4444'
else:
    v_tot, n_c, p_t, p_roi, g_tot_bs, g_tot_u, v_usd = 0, "N/A", 0, 0, 0, 0, 0
    c_roi_g = '#ef4444'

resumen_html = f"<div class='summary-box'><div class='summary-header'>🏆 RESUMEN GLOBAL DEL DÍA</div><div class='summary-grid'><div class='summary-item'><span style='font-size:11px;color:#6b7280;font-weight:800;display:block;'>🔄 Vueltas</span><span style='font-size:15px;color:var(--text-color);font-weight:900;'>{v_tot} <span style='font-size:11px;'>({n_c})</span></span></div><div class='summary-item'><span style='font-size:11px;color:#6b7280;font-weight:800;display:block;'>💸 Volumen Movido</span><span style='color:#0ea5e9;font-size:17px;font-weight:900;'>${v_usd:,.2f}</span></div><div class='summary-item'><span style='font-size:11px;color:#6b7280;font-weight:800;display:block;'>📈 Tasa Promedio</span><span style='font-size:15px;color:var(--text-color);font-weight:900;'>Bs.{p_t:,.2f}</span></div><div class='summary-item'><span style='font-size:11px;color:#6b7280;font-weight:800;display:block;'>🚀 ROI Promedio</span><span style='font-size:15px;font-weight:900;color:{c_roi_g};'>{p_roi:,.2f}%</span></div><div class='summary-item-full'><span style='font-size:11px;color:#0ea5e9;font-weight:800;display:block;margin-bottom:4px;'>💰 GANANCIA TOTAL HOY</span><div style='display:flex;justify-content:center;gap:10px;'><span style='color:#10b981;font-size:22px;font-weight:900;'>Bs.{g_tot_bs:,.2f}</span><span style='color:var(--text-color);font-weight:900;font-size:15px;align-self:center;'>≈₮{g_tot_u:,.2f}</span></div></div></div>{breakdown_html}</div>"
...
with st.expander("📅 CENTRO DE RÉCORDS Y AUDITORÍA"):
    ...
    if not st.session_state.historial_df.empty:
        ...
        if not df_view.empty:
            ...
            records_panel = f"<div style='display:flex; justify-content:space-around; background-color: var(--secondary-background-color); padding:15px; border-radius:12px; border:1px dashed #0ea5e9; margin-top:15px; margin-bottom:15px;'><div style='text-align:center;'><span style='font-size:10px;color:#6b7280;font-weight:800;display:block;margin-bottom:2px;'>VOLUMEN (USD)</span><span style='font-size:18px;color:#0ea5e9;font-weight:900;'>${t_usd_view:,.2f}</span></div><div style='text-align:center; border-left:1px solid rgba(128,128,128,0.2); border-right:1px solid rgba(128,128,128,0.2); padding:0 15px;'><span style='font-size:10px;color:#6b7280;font-weight:800;display:block;margin-bottom:2px;'>GANANCIA NETA</span><span style='font-size:18px;color:#10b981;font-weight:900;'>Bs.{t_gan_view:,.2f}</span></div><div style='text-align:center;'><span style='font-size:10px;color:#6b7280;font-weight:800;display:block;margin-bottom:2px;'>ROI PROMEDIO</span><span style='font-size:18px;color:#f59e0b;font-weight:900;'>{p_roi_view:,.2f}%</span></div></div>"
            st.markdown(records_panel, unsafe_allow_html=True)
